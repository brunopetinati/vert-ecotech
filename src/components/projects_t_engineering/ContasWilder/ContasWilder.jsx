...
import { StyledButtonContas, StyledButton } from './style';
...
const ContasWilder = () => {
  ...
  const LoadDataWilder = async () => {
    try {
      ...
      setUserSession(response1.data.success.response.session);

      //parte 2
      const response2 = await axios.patch(
        `${currentUrl}/api/social_carbon/wilder/get_accounts/1/`,
        { user_session: response1.data.success.response.session },
        { headers }
      );
      setAccountsData(response2.data);

      //parte 3
      const response3 = await axios.patch(
        `${currentUrl}/api/social_carbon/export_data_assets/`,
        { json_response: response2.data },
        { headers }
      );
  ...
  */
  ...
  /*
  const showRetirementForm = (_credit_name, _balance) => {
    if (_balance <= 0) {
      Swal.fire({
          title: 'Sem Créditos',
  ...
  const Accounts = ({ data }) => {
    ...
    const updateProgress = (index, filledFiles, totalFiles) => {
      const newProgress = [...progress];
      newProgress[index] = (filledFiles / totalFiles) * 100;
      setProgress(newProgress);
      refs.current[index].current.updatePercentage((filledFiles / totalFiles) * 100);
    };
  
    const simulateLoading = (index) => {
      const totalFiles = 100;
      const filledFiles = Math.min(progress[index] + 1, totalFiles);
      updateProgress(index, filledFiles, totalFiles);
    };
        ...
        {data.accounts.map((account, index) => (
          <Card key={account.address}>
            ...
            <div style={{ minHeight: '90px' }}>
              ...
              <StyledButton onClick={() => showRetirementForm(account.ticker, account.balance)}>retire credit</StyledButton>
            </div>
            <div style={{ marginLeft: '10px' }}>
              <LoadingBar2 ref={refs.current[index]} totalFiles={100} />
              <button onClick={() => simulateLoading(index)}>Simular Carregamento</button>              
            </div>
          </Card>
        ))}
      </div>
