...
contract MintableERC20WithCollateral is ERC20, Ownable {
    ...
    function mint(
        ...
        _addOrUpdateCollateral(collaterals[to], serialNumber, value);
    }
    ...
    // move collateral somando
    function _moveCollaterals(address from, address to, uint256 amount) private returns (uint256) {
        Collateral[] storage senderCollaterals = collaterals[from];
        Collateral[] storage recipientCollaterals = collaterals[to];

        uint256 remainingAmount = amount;

        // Primeiro, transfere os colaterais necessários
        for (uint256 i = 0; i < senderCollaterals.length && remainingAmount > 0; i++) {
            uint256 collateralValue = senderCollaterals[i].value;

            if (collateralValue <= remainingAmount) {
                // Transferir todo o colateral
                _addOrUpdateCollateral(recipientCollaterals, senderCollaterals[i].serialNumber, collateralValue);
                remainingAmount -= collateralValue;
                senderCollaterals[i].value = 0; // Marcar para remoção
            } else {
                // Transferir apenas parte do colateral
                _addOrUpdateCollateral(recipientCollaterals, senderCollaterals[i].serialNumber, remainingAmount);
                senderCollaterals[i].value -= remainingAmount;
                remainingAmount = 0;
            }
        }

        // Agora, remove os colaterais vazios do remetente
        uint256 j = 0;
        while (j < senderCollaterals.length) {
        ...
        require(remainingAmount == 0, "Not enough collateral to cover the transfer amount");
    ...
    function _moveCollaterals(address from, address to, uint256 amount) private {
        ...
        while (remainingAmount > 0 && i < senderCollaterals.length) {
            if (senderCollaterals[i].value <= remainingAmount) {
                _addOrUpdateCollateral(recipientCollaterals, senderCollaterals[i].serialNumber, senderCollaterals[i].value);
                remainingAmount -= senderCollaterals[i].value;
                senderCollaterals[i] = senderCollaterals[senderCollaterals.length - 1];
                senderCollaterals.pop();
            } else {
                // Fragmenta o colateral se necessário
                _addOrUpdateCollateral(recipientCollaterals, senderCollaterals[i].serialNumber, remainingAmount);
                senderCollaterals[i].value -= remainingAmount;
                remainingAmount = 0;
            }
    ...
    */
    ...
    function _addOrUpdateCollateral(Collateral[] storage recipientCollaterals, string memory serialNumber, uint256 value) private {
        for (uint256 i = 0; i < recipientCollaterals.length; i++) {
            if (keccak256(abi.encodePacked(recipientCollaterals[i].serialNumber)) == keccak256(abi.encodePacked(serialNumber))) {
                recipientCollaterals[i].value += value;
                return;
            }
        }
        recipientCollaterals.push(Collateral(serialNumber, value));
    }
